...
This repository contains a PyTorch implementation of **RWKVHSI**, a lightweight RWKV-based model for hyperspectral image classification. The project includes the model code, experiment entry point, dataset loaders, metric utilities, local benchmark datasets, and a reproducible pip environment file.
...
Files ending in `~` and files under `__pycache__/` are backup/cache artifacts and are not required for normal training or evaluation.
...
By default, `main.py` reads datasets from `./Datasets/`.

The currently present dataset files are:
...
Runnable dataset choices with the current loaders:

- `--dataset IndianPines`
- `--dataset whulk`

The local `Houston2013` files are present, but they do not directly match the configured `Houston` or `GRSS` loader paths in `datasets.py`. In particular, the current loaders expect different folder names, file names, and variable names. To run Houston2013, rename the files and adjust the `.mat` variables to match `datasets.py`, or update the corresponding loader branch.
...
| --- | --- | --- |
| `--dataset` | Dataset name. Local runnable choices include `IndianPines` and `whulk`. | `IndianPines` |
...
| `--download` | Download supported public datasets listed in `datasets.py`. | `None` |
